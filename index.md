...
# <a id="top"></a>Zotero 介紹

## <a id="features"></a>特點 Features
這是一個
...
- **免費**且**開源**
- 可多裝置**同步**
- 可方便地在 PDF 檔案或是網頁上面註記
- 有很多開源且免費的插件可以擴充功能

的文獻管理軟體

## <a id="what-can-zotero-do"></a>有了 Zotero 能做什麼？
### <a id="import-ref"></a>快速下載文獻
...
### <a id="export-bibtex-for-latex"></a>導出 BibTeX for LaTeX
...
### <a id="copy-citation"></a>複製格式化後的引用資料
...
### <a id="zotero-and-connector"></a>Zotero 電腦版與瀏覽器插件
...
### <a id="zotero-on-phones-and-tablets"></a>Zotero on 手機/平板

## <a id="usage"></a>基礎使用

### 1. 新增 Collection
在左上角有一個像資料夾的圖示，相當於幫 project 或是正在撰寫的論文開一個資料夾，來存放和它相關的文獻。
點進去這個 collection 後，任何從 [Zotero Connector](#zotero-and-connector) 下載的文件會自動收納在這個 collection 底下。

![](attachment/a8ef82a45616dd3a1778acdfbb586148.png)

### 2. 新增文獻
除了[直接從瀏覽器導入](#import-ref)之外，也可以把電腦上的檔案拖進 collection 中，會自動嘗試抓取 metadata 和 PDF。
...
### 3. 在文獻上做筆記
請參考[這一節](#read-ref)。若想使用 Zotero 來做筆記，請開啟設定 (MacOS 快捷鍵：`command` + `,`)，並將 Reader 中的 `Open PDFs using` 以及 `Open snapshots using` 設定成 `Zotero`。
...
#### <a id="overleaflatex"></a>Overleaf/LaTeX
以在 Overleaf 上編輯 `.bib` 檔為例。請先[下載 Better BibTeX 插件](#better-bibtex)。
...
這是匯入文獻當下 Zotero 自己產生的副本。點開會發現上面並沒有註記，這是因為 Zotero 並不會直接去更改原始檔。若想與他人分享筆記，請回到 Zotero 點選想匯出的 PDF，並選擇 `File -> Export PDF...`
...
Zotero 有許多開源的插件，絕大部分的原始碼可以在 GitHub 上看到。

> [!caution] 關於第三方插件
> 雖然絕大多數的插件皆開源且免費取用，但仍有部分插件有付費後才會開放的功能。
> 
> 另外，有一些插件需要提供 API Keys 或是其它私人資訊，請先看過 GitHub 頁面相關的討論來判斷此插件是否安全。

以下以 Better BibTeX 插件為例，示範如何安裝插件。

### <a id="better-bibtex"></a>Better BibTeX
[GitHub 頁面](https://github.com/retorquere/zotero-better-bibtex)

1. 進入 GitHub 頁面後點擊 `Releases`
...
### <a id="translate-for-zotero"></a>Translate for Zotero
安裝插件方式請參考[以上教學](#better-bibtex)

- [GitHub 頁面](https://github.com/windingwind/zotero-pdf-translate#readme)
- 若想使用自己的 API key，請去 Zotero 的設定輸入 key。

  ![](attachment/8f73a005230f49f20c1708c4f2a1cc90.png)

### <a id="obsidian-notes-for-zotero"></a>Obsidian Notes for Zotero
...
任何人皆可自由引用、修改，但請務必保留此版權聲明，並以相同授權開放。 [↑ 回頂端](#top)
